...
public class TeacherDAOImpl implements ITeacherDAO {
    ...
    public Teacher insert(Teacher teacher) throws TeacherDAOException {
        String sql = "INSERT INTO teachers (firstname, lastname, vat, fathername, phone_num, email, street," +
                " street_num, zipcode, city_id, uuid, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?";

        Teacher insertedTeacher = null;

        try (Connection connection = DBUtil.getConnection();
             PreparedStatement ps = connection.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {    // Προγραμματιστικό insert είναι το ps. Το string είναι το relational
            ...
            ps.setString(11, String.valueOf(UUID.randomUUID()));
            ps.setTimestamp(12, Timestamp.valueOf(LocalDateTime.now()));
            ps.setTimestamp(13, Timestamp.valueOf(LocalDateTime.now()));
            ps.executeUpdate();

            ResultSet rsGeneratedKeys = ps.getGeneratedKeys();    // Τα αποτελέσματα απο το query μπαίνουν σε Result set είναι πίνακας
            if (rsGeneratedKeys.next()) {
                int generatedKey = rsGeneratedKeys.getInt(1);  // getter του result set
    ...
    public Teacher update(Teacher teacher) throws TeacherDAOException {
        String sql = "UPDATE teachers SET firstname = ? , lastname = ?, vat = ?, fathername = ?, phone_num = ?, email = ?, street = ?," +
                " street_num = ?, zipcode = ?, city_id = ?, updated_at = ? WHERE id = ?";
        ...
        try (Connection connection = DBUtil.getConnection();
            ...
            if (rs.next()) {
                teacher = new Teacher(rs.getInt("id"), rs.getString("firstname"), rs.getString("lastname"),
                                        ...
                                        rs.getString("phone_num"), rs.getString("email"),
                                        rs.getString("street"), rs.getString("street_num"),
    ...
    public List<Teacher> getAll() throws TeacherDAOException {
        ...
        try (Connection connection = DBUtil.getConnection();
            ...
            e.printStackTrace();
            //logging
            throw new TeacherDAOException("SQL Error. Error in getting all teachers.");
        }
    }
    ...
    public List<Teacher> getByLastname(String lastname) throws TeacherDAOException {
        ...
        try (Connection connection = DBUtil.getConnection();
            ...
            e.printStackTrace();
            //logging
            throw new TeacherDAOException("SQL Error. Error in getting teachers by lastname.");
        }
    }
